...
function getSession()  { try { return JSON.parse(sessionStorage.getItem(SK_SESSION)); } catch { return null; } }
function setSession(u) { sessionStorage.setItem(SK_SESSION, JSON.stringify(u)); }
function clearSession(){ sessionStorage.removeItem(SK_SESSION); }
function isAdmin()     { const s = getSession(); return s && s.role === 'admin'; }

/* ── Rate limiting de login ── */
function getLoginFails()       { return parseInt(localStorage.getItem(SK_LOGIN_FAILS) || '0', 10); }
function incrementLoginFails() { localStorage.setItem(SK_LOGIN_FAILS, getLoginFails() + 1); }
function resetLoginFails()     { localStorage.removeItem(SK_LOGIN_FAILS); localStorage.removeItem(SK_LOGIN_BLOCK); }
function isLoginBlocked() {
  const until = parseInt(localStorage.getItem(SK_LOGIN_BLOCK) || '0', 10);
  if (Date.now() < until) return true;
  if (until && Date.now() >= until) { localStorage.removeItem(SK_LOGIN_BLOCK); localStorage.removeItem(SK_LOGIN_FAILS); }
  return false;
}
function blockLogin() {
...
function mostrarReauth() {
  return new Promise(resolve => {
    ...
    inputPass.value = ''; errEl.textContent = '';
...
function mostrarColabHoraModal(tipo) {
  return new Promise(resolve => {
    ...
    horaInput.value = getNowTimeStr(); errEl.textContent = '';
    ...
    const onConfirmar = () => {
      ...
      overlay.classList.remove('active'); cleanup(); resolve(hora);
    };
...
async function mostrarApp(session) {
  ...
  document.querySelectorAll('.colab-only').forEach(el => el.classList.toggle('hidden', isAdmin()));
...
function iniciarContadorBloqueo(errorBox) {
  ...
  blockInterval = setInterval(() => {
    ...
    if (secs <= 0) {
      ...
      errorBox.textContent = `Demasiados intentos. Espera ${secs} segundo${secs !== 1 ? 's' : ''} para intentar de nuevo.`;
    }
  }, 1000);
}
...
async function getMesesAsistencia() {
  const { data, error } = await supabaseClient
    .from('meses_asistencia').select('*').order('nombre', { ascending: true });
  if (error) { console.error(error); return []; }
  return data || [];
}
...
overlayAsist?.addEventListener('click', e => { if (e.target === overlayAsist) overlayAsist.classList.remove('active'); });
...
document.getElementById('modalAsistMesConfirmar')?.addEventListener('click', async () => {
  ...
  inputMesAsist.classList.remove('input-error'); errEl.textContent = '';

  if (!nombre || nombre.length < 2) {
    errEl.textContent = 'Mínimo 2 caracteres.'; inputMesAsist.classList.add('input-error'); return;
  }
  const meses = await getMesesAsistencia();
  if (meses.find(m => m.nombre.toLowerCase() === nombre.toLowerCase())) {
    errEl.textContent = 'Ya existe un mes con ese nombre.'; inputMesAsist.classList.add('input-error'); return;
  }
...
async function renderColabAsistencia() {
  ...
  if (headerFecha) headerFecha.textContent = new Date().toLocaleDateString('es-PE', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
  ...
  if (!registro) {
    ...
    btnEntrada.disabled = false; btnSalida.disabled = true;
    ...
    btnEntrada.disabled = true; btnEntrada.title = 'Ya registraste tu entrada hoy';
    btnSalida.disabled  = false; btnSalida.title  = '';
  } else if (registro.entrada && registro.salida) {
    ...
    btnEntrada.disabled = true; btnSalida.disabled = true;
  }
}
...
document.getElementById('btnColabEntrada').addEventListener('click', async () => {
  ...
  const mesId = getMesActivoAsistencia();
  if (!mesId) {
    mostrarToast('No hay mes de asistencia activo. Contacta al administrador.', 'error'); return;
  }
  ...
  const nuevo = { nombre: s.display, entrada: hora, salida: null, registrado_por: s.display, fecha: getTodayStr(), mes_asist_id: mesId };
  ...
  await audit('add', `Entrada registrada por colaborador: ${s.display} (${hora}) — Mes: ${mesId}`);
...
document.getElementById('btnColabSalida').addEventListener('click', async () => {
  ...
  if (!hora) return;
  if (hora === registro.entrada) { mostrarToast('La salida no puede ser igual a la entrada.', 'error'); return; }
...
async function delAsistencia(id) {
  if (!isAdmin()) return;
  const confirmado = await mostrarConfirm('¿Eliminar registro?', 'Se eliminará este registro de asistencia. Esta acción no se puede deshacer.', 'Eliminar');
  if (!confirmado) return;
...
document.getElementById('formAsistencia').addEventListener('submit', async function (e) {
  ...
  if (accion === 'entrada') {
    ...
    if (yaExiste) { mostrarToast(`Ya existe un registro para ${nom.value.trim()} hoy.`, 'error'); return; }

    const preview = `
      <strong>Trabajador:</strong> ${sanitize(nom.value.trim())}<br>
      <strong>Hora entrada:</strong> ${ent.value}${sal.value ? `<br><strong>Hora salida:</strong> ${sal.value}` : ''}<br>
      <strong>Fecha:</strong> ${hoy}<br>
      <strong>Registrado por:</strong> ${sanitize(s?.display || '—')}
    `;
    const confirmado = await mostrarGuardar('¿Confirmar registro de entrada?', preview);
    if (!confirmado) return;

    const nuevo = { nombre: nom.value.trim(), entrada: ent.value, salida: sal.value || null, registrado_por: s?.display || '—', fecha: hoy, mes_asist_id: mesId };
    const { error } = await supabaseClient.from('asistencia').insert(nuevo);
    ...
    this.reset(); toggleAsistenciaMode();
    ...
    const preview = `
      <strong>Trabajador:</strong> ${sanitize(reg.nombre)}<br>
...
document.getElementById('clearAsistencia').addEventListener('click', async () => {
  ...
  const confirmado = await mostrarConfirm(
    ...
    'Se eliminarán TODOS los registros de asistencia de este mes. Esta acción no se puede deshacer.',
    'Limpiar todo'
  );
  if (!confirmado) return;
...
async function getMeses() {
  const { data, error } = await supabaseClient.from('meses').select('*').order('nombre', { ascending: true });
  if (error) { console.error(error); return []; }
  return data || [];
}

async function getDias(mesId) {
  const { data, error } = await supabaseClient.from('dias').select('*').eq('mes_id', mesId).order('fecha', { ascending: true });
  if (error) { console.error(error); return []; }
  return data || [];
}
...
function actualizarReporte(lista) {
  ...
  document.getElementById('reporteGananciaTotal').style.color  = totalB >= 0 ? 'var(--success)' : 'var(--danger)';
}

async function delDia(mesId, id) {
  if (!isAdmin()) return;
  const confirmado = await mostrarConfirm('¿Eliminar este día?', 'Se eliminará permanentemente el registro de este día del mes activo.', 'Eliminar');
  if (!confirmado) return;
  ...
  await audit('delete', `Día eliminado: ${reg?.fecha} — Total neto: ${fmt(totalBruto - gastos)}`);
  await renderVentas();
  await renderSelectMesesDescargas();
...
document.getElementById('formVentas').addEventListener('submit', async function (e) {
  ...
  clearErrors(['err-fecha', 'err-efectivo', 'err-yape', 'err-plin', 'err-transf', 'err-ganancia'], [fec, efectivo, yape, plin, transf, ganancia]);
  ...
  if (existe) {
    ...
    const confirmado = await mostrarConfirm(`¿Sobreescribir ${fec.value}?`, `Ya existe un registro para esta fecha en el mes "${mesActual.nombre}". ¿Deseas reemplazarlo?`, 'Sobreescribir');
    if (!confirmado) return;

    const { error } = await supabaseClient.from('dias').update({ efectivo: eNum, yape: yNum, plin: pNum, transf: tNum, ganancia: gNum, registrado_por: s?.display || '—' }).eq('id', existe.id);
    ...
    const preview = `
      <strong>Mes:</strong> ${sanitize(mesActual.nombre)}<br>
    ...
    const nuevoDia = { mes_id: mesId, fecha: fec.value, efectivo: eNum, yape: yNum, plin: pNum, transf: tNum, ganancia: gNum, registrado_por: s?.display || '—' };
  ...
  actualizarPreview();
  const hoy = new Date();
  document.getElementById('fechaVenta').value = `${hoy.getFullYear()}-${String(hoy.getMonth()+1).padStart(2,'0')}-${String(hoy.getDate()).padStart(2,'0')}`;
});
...
document.getElementById('clearDia').addEventListener('click', async () => {
  ...
  const meses = await getMeses();
  const mes   = meses.find(m => m.id === mesId);
  const confirmado = await mostrarConfirm(`¿Limpiar el mes "${mes?.nombre}"?`, 'Se eliminarán todos los días registrados de este mes. Esta acción no se puede deshacer.', 'Limpiar mes');
  if (!confirmado) return;
...
document.getElementById('clearAuditoria').addEventListener('click', async () => {
  if (!isAdmin()) return;
  const confirmado = await mostrarConfirm('¿Eliminar historial de auditoría?', 'Se borrará todo el registro de acciones del sistema. Esta operación es irreversible.', 'Eliminar historial');
  if (!confirmado) return;
...
document.getElementById('btnNuevoMes').addEventListener('click', () => {
  inputMes.value = ''; document.getElementById('err-mes').textContent = '';
...
function exportarExcel(filename, sheetName, headers, rows) {
  if (typeof XLSX === 'undefined') { mostrarToast('Error: librería Excel no cargada.', 'error'); return; }
...
document.getElementById('btnDescargarVentas').addEventListener('click', async () => {
  ...
  sorted.forEach(d => {
    ...
    rows.push([d.fecha, dia, +(d.efectivo || 0).toFixed(2), +(d.yape || 0).toFixed(2), +(d.plin || 0).toFixed(2), +(d.transf || 0).toFixed(2), +bruto.toFixed(2), +neto.toFixed(2), +gan.toFixed(2), +balance.toFixed(2), margen, d.registrado_por || '—']);
  });
  ...
  rows.push([]);
  rows.push(['TOTAL', '', +totalE.toFixed(2), +totalY.toFixed(2), +totalP.toFixed(2), +totalT.toFixed(2), +totalB.toFixed(2), +totalN.toFixed(2), +totalG.toFixed(2), +totalBal.toFixed(2), totalN > 0 ? ((totalG / totalN) * 100).toFixed(1) + '%' : '0%', '']);
...
/* ── Descargar asistencias MENSUALES ── */
document.getElementById('btnDescargarAsistencias').addEventListener('click', async () => {
  ...
  const { data: asistencias, error } = await supabaseClient
    .from('asistencia').select('*').eq('mes_asist_id', mesId)
    .order('fecha', { ascending: true }).order('entrada', { ascending: true });

  if (error || !asistencias?.length) { mostrarToast('No hay asistencias para este mes.', 'info'); return; }

  const headers = ['#', 'Trabajador', 'Fecha', 'Día', 'Hora Entrada', 'Hora Salida', 'Duración', 'Registrado por'];
  const rows = asistencias.map((r, i) => {
    const fecha = r.fecha ? new Date(r.fecha + 'T12:00:00') : null;
    const dia   = fecha ? fecha.toLocaleDateString('es-PE', { weekday: 'short' }).toUpperCase() : '—';
  ...
  asistencias.forEach(r => {
    if (!trabajadoresMap[r.nombre]) trabajadoresMap[r.nombre] = { dias: 0, completos: 0, minutos: 0 };
  ...
  await audit('add', `Exportación Excel de asistencias mensuales: ${mes?.nombre}`);
